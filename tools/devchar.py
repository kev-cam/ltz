...
def emit_diode_so_source(db: sqlite3.Connection, model_name: str) -> str:
    """Generate C++ for a DIODE .so plugin (PyMS/Xyce ABI) -- a 1-D I(Vd) table.

    This is the table-driven FALLBACK for exponential diodes (and for the bfit
    --merge'd models that contain them): an interpolated, smooth, BOUNDED I-V
    that converges where limexp->exp does not (the exp loses its current limiting
    in PyMS). Same approach as the VDMOS/BJT table models, one dimension.

    Nodes: V[0]=anode, V[1]=cathode.  F[0]=+Id (anode), F[1]=-Id (cathode).
    Jacobian via finite differences on the table; Q = Cj*Vd for the junction cap.
    """
    row = db.execute("SELECT params FROM models WHERE name=?", (model_name,)).fetchone()
    if not row:
        raise ValueError(f"Model {model_name} not found")
    params = json.loads(row[0])

    sweep = db.execute(
        "SELECT columns, data FROM sweeps WHERE model=? AND sweep_type='id_vd'",
        (model_name,)
    ).fetchone()
    if not sweep:
        raise ValueError(f"No id_vd sweep data for {model_name}")

    columns = json.loads(sweep[0])
    data = list(struct.iter_unpack(f'<{len(columns)}d', sweep[1]))
    # columns: [Va_sweep(=V(a)=Vd), I(Va), V(a)] ; Id = -I(Va) (forward positive)
    ci = next((k for k, c in enumerate(columns)
               if c.lower().replace(' ', '').startswith('i(')), 1)
    tbl = {}
    for r in data:
        tbl[r[0]] = -r[ci]            # Vd = swept V(a); Id = -I(Va)
    vd_set = sorted(tbl)
    max_n = 256
    if len(vd_set) > max_n:
        step = max(1, len(vd_set) // max_n)
        vd_set = vd_set[::step]
    n = len(vd_set)
    cjo = params.get('cjo', 0) or params.get('cj0', 0) or 0

    cpp = f"""// {model_name} -- diode table model (PyMS/Xyce ABI)
// Auto-generated by devchar. {n}-point I(Vd) linear-interpolation table.
// Table-driven fallback for exponential diodes: smooth + bounded -> converges
// where limexp->exp does not.  Nodes: V[0]=anode, V[1]=cathode.
#include <cstring>
#include <cmath>
...
static const double vd_bp[{n}] = {{
    {', '.join(f'{v:.8e}' for v in vd_set)}
}};
static const double id_tbl[{n}] = {{
    {', '.join(f'{tbl[v]:.8e}' for v in vd_set)}
}};
...
SO_EMITTERS = {
    ...
    'D': emit_diode_so_source,
    'DIODE': emit_diode_so_source,
}
